...
% Smooth_window = 5000;
% amp = conv(amp,ones(1,Smooth_window)/Smooth_window);
% amp = conv(amp,ones(1,Smooth_window)/Smooth_window);
% amp = diff(amp);
% z = hilbert(amp);
% amp = abs(z);
...
% tmp = zeros(length(trigger),1);
% tmp_index = trigger_index(1);
% curr_trigger = trigger_index(1);
% for ii = 2:length(trigger_index)
%     if (trigger_index(ii)>(curr_trigger+5000))&&(trigger_index(ii)-trigger_index(ii-1)>2500)
%         tmp_index = [tmp_index,trigger_index(ii)];
%         curr_trigger = trigger_index(ii);
%     end
% end
% tmp(tmp_index) = max(amp)/4;

% Smooth_window = 5000;
% env = conv(amp(1:2e6),ones(1,Smooth_window)/Smooth_window);
...
% %%
% % tmp_index = [];
% % for ii=1:length(alarm)-2
% %     if alarm(ii)==0&&alarm(ii+1)==1
...
%     Smooth_window = floor(0.05*fs);
%     XX_d = conv(XX_d,ones(1,Smooth_window)/Smooth_window);
